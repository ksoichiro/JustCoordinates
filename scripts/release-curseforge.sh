...
MOD_ID="justcoordinates"
CURSEFORGE_PROJECT_ID="1459678"
PROJECT_NAME="Just Coordinates"
# ---------------------------------
...
# Build game version IDs array (include Quilt for Fabric builds)
if [ "$LOADER" = "fabric" ]; then
  QUILT_VERSION_ID=$(echo "$VERSIONS_JSON" | jq \
    '[.[] | select(.name == "Quilt")] | first | .id // empty')
  if [ -n "$QUILT_VERSION_ID" ]; then
    GAME_VERSION_IDS="[$GAME_VERSION_ID, $LOADER_VERSION_ID, $QUILT_VERSION_ID]"
  else
    GAME_VERSION_IDS="[$GAME_VERSION_ID, $LOADER_VERSION_ID]"
  fi
else
  GAME_VERSION_IDS="[$GAME_VERSION_ID, $LOADER_VERSION_ID]"
fi
